...
function DraftModel({ setToggleModel }) {
      ...
      <div className="bg-white rounded-lg w-2/4 max-w-2/4">
        <div className="flex flex-col items-start p-4">
          <div className="flex items-center w-full justify-between border-b pb-1">
            <div className="text-gray-900 font-medium text-lg">{`Draft ${MOCK_DRAFT.length}/20`}</div>
            <HiX
          ...
          {draftLists &&
            draftLists.map((item) => <DraftCard key={item.id} item={item} />)}
          <hr />
